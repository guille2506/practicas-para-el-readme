...
un dato curioso si nosotros guardamos un dni 2 veces en la db se genera un error el cual no permite guardar , porque se supone que no deberia haber dni duplicados , esto sucede porque  se implementaron reglas en  la tabla miembros porque tiene  un índice único en dni, (gif mostrando el problema)
